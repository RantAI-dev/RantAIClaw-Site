...
The wizard walks you through six decisions, each with a sensible default. It writes a profile under `~/.rantaiclaw/profiles/default/` (or whichever profile is active — see [Profiles](/docs/concepts/profiles)).
...
|---|---|
| **Manual** | Narrow auto-approve list, strict command allowlist; almost everything prompts |
| **Smart** | Curated auto-approve list (read tools + safe shell patterns like `ls *`, `cat *`, `git status`) |
| **Strict** | Deny-by-default, allows only the Smart read set + memory writes + skill installs + cron / session ops |
| **Off** | Runtime `level = "full"`; auto-approves everything except forbidden paths and high-risk commands |

Smart is the recommended starting default. See [Autonomy Levels](/docs/concepts/autonomy-levels) for the full mechanics — the runtime enum has only three values (`read_only`, `supervised`, `full`); the four preset names are bundles the wizard writes.
