...
- **`/mcp/servers` HTTP endpoint.** Doesn't exist. The closest planned endpoint is `GET /config/mcp-servers` in `src/gateway/config_api.rs`, but that file isn't mounted on the router (see [Gateway](/docs/concepts/gateway)).
